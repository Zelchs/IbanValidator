...
public enum IbanPrefix {
    ...
    public static int getIbanLengthByCountryCode(String countryCode) {
        IbanPrefix prefix = getByCountryCode(countryCode);
        if (prefix != null) {
            return prefix.getIbanLength();
        }
        return -1;
    }

    private static IbanPrefix getByCountryCode(String countryCode) {
        for (IbanPrefix prefix : values()) {
            if (prefix.name().equals(countryCode.toUpperCase())) {
                return prefix;
            }
        }
        return null;
